...
class FireworksWebGPUPlugin {
    ...
    registerSocketHandlers() {
        ...
        if (!this.fpsUpdateHandler) {
            this.fpsUpdateHandler = (socket) => {
                ...
                socket.on('fireworks-webgpu:fps-update', (data) => {
    ...
    registerRoutes() {
        ...
        this.api.registerRoute('post', '/api/fireworks-webgpu/benchmark/set-preset', (req, res) => {
            try {
        ...
        this.api.registerRoute('get', '/api/fireworks-webgpu/benchmark/fps', (req, res) => {
            try {
                // FPS is tracked in the overlay's GPU engine
                // We'll use socket.io to request current FPS
                this.api.emit('fireworks-webgpu:request-fps');
        ...
        this.api.registerRoute('post', '/api/fireworks-webgpu/benchmark/restore', (req, res) => {
            try {
    ...
    async destroy() {
        ...
        if (this.connectedSockets) {
            this.connectedSockets.forEach(socket => {
                socket.removeAllListeners('fireworks-webgpu:fps-update');
            });
